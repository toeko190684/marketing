...
if($_GET[data]=='reconumber'){
	...
	$kode_promo = mysql_query("SELECT ifnull( concat( 'RC/MKI/', '$_POST[divisi]', '/', '$_POST[departemen]', '/', substring($tgl[2],3,2), 
							  reverse( substring( reverse( concat( '00', $tgl[0] ) ) , 1, 2 ) ) , 
							  reverse( substring( reverse( concat( '0000', MAX( reverse( substring( reverse( kode_promo ) , 1, 4 ) ) ) +1 ) ) , 1, 4 ) ) ) , 
							  concat( 'RC/MKI/', '$_POST[divisi]', '/', '$_POST[departemen]', '/', substring($tgl[2],3,2), 
							  reverse( substring( reverse( concat( '00', $tgl[0] ) ) , 1, 2 ) ) , '0001' ) ) AS kode_promo
							  FROM RECO_REQUEST WHERE trim( reverse( substring( reverse( kode_promo ) , 10, 20 ) ) )
							  = concat( 'RC/MKI/', '$_POST[divisi]', '/', '$_POST[departemen]' )");
...
if($_GET[data]=='simpanreco'){
	...
	if($cek>0){
		...
		$sql = mysql_query("update reco_request set kode_promo='$_POST[kode_promo]',
														...
														background = replace('$_POST[background]','-',' '),
														promo_mechanisme = replace('$_POST[promo_mechanishm]','-',' '),
														claim_mechanisme = replace('$_POST[claim_mechanishm]','-',' '),
        ...
        if($rbudget>0){		
			$sql = mysql_query("insert into reco_request(kode_promo,
												...
												values('$_POST[kode_promo]',
														...
														replace('$_POST[background]','-',' '),
														replace('$_POST[promo_mechanishm]','-',' '),
														replace('$_POST[claim_mechanishm]','-',' '),
														'$_POST[claimtradeoff]',
														$_POST[costofpromo],
														'$_POST[typeofcost]',
														'$_POST[costrasio]',
...
if($_GET[data]=='json_coa'){
	$coa = "select account_id,account_name,substring(created_by,1,1)as tipe_biaya,substring(created_by,2,1)as typeofcost 
	       from kinosentraacc.dbo.account  where substring(created_by,1,1) in ('O','P')order by account_id";
...
if($_GET[data]=='json_outstanding'){
	...
	$partial = mysql_query("select partial_ammount from reco_request where kode_promo='$_POST[id]'");
	$rpartial = mysql_fetch_array($partial);
	echo $rpartial[partial_ammount]+$r[total];
}
		  ...
		  if($rcari[status]<>'pending'){
				  ...
				  if($sql){					
						if($_POST[status]=='approved'){	
							$sql_po = "select count(*) as jml_po from kinosentraacc.dbo.epo where po_id='$rcari[po_so_number]'";
							$qsql_po = odbc_exec($conn2, $sql_po);
							$rsql_po = odbc_fetch_array($qsql_po);
							if($rsql_po[jml_po] == 0){						
								...
								echo "claim Number System : $_POST[id] berhasil di approved..!, Jurnal ID tidak terbentuk untuk PO ID : $rcari[po_so_number]";
							}
						}else{
